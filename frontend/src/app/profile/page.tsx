  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#FBF6EE] flex flex-col items-center justify-center py-20 text-center animate-pulse font-sans">
        ...
        <p className="text-sm font-medium text-[#6E8B8B]">Loading your health profile...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FBF6EE] text-[#24322F] p-4 md:p-8 font-sans">
      <div className="max-w-3xl mx-auto space-y-6">
        {/* Header Bar */}
        <header className="rounded-2xl border border-[#D8CDBB] bg-white p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-[#1F5B5B] flex items-center justify-center shadow-md">
              <HeartPulse className="h-6 w-6 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold tracking-tight text-[#1F5B5B]">Health Profile Editor</h1>
              <p className="text-xs text-[#6E8B8B]">Fill in demographic, stats, and lifestyle to unlock planning</p>
            </div>
          </div>

          <div className="flex items-center gap-4 shrink-0">
            <div className="text-right">
              <span className="text-xs font-bold text-[#6E8B8B] uppercase tracking-wider">Completion Status</span>
              <div className="text-lg font-extrabold text-[#1F5B5B] mt-0.5">{completionPercentage}%</div>
            </div>
            <div className="w-24 bg-[#E5E9E7] rounded-full h-3.5 overflow-hidden">
              <div
                className="bg-[#1F5B5B] h-3.5 rounded-full transition-all duration-500"
        ...
        {completionPercentage === 100 && (
          <div className="rounded-2xl bg-[#DCEEE6] text-[#1F5B5B] p-6 border border-[#1F5B5B]/15 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              <Award className="h-8 w-8 text-[#1F5B5B] shrink-0 mt-0.5" />
              <div>
                <h3 className="text-base font-bold">Your profile is 100% complete!</h3>
                <p className="text-xs text-[#1F5B5B]/90 mt-1 leading-relaxed">
            ...
            <button
              ...
              className="bg-[#1F5B5B] hover:bg-[#174646] text-white font-semibold text-xs py-3 px-5 rounded-xl flex items-center justify-center gap-1.5 transition-colors cursor-pointer shrink-0 shadow-sm"
            >
        ...
        {/* Form Body */}
        <form onSubmit={handleSubmit} className="rounded-2xl border border-[#D8CDBB] bg-white p-6 shadow-sm space-y-6">
          {/* Section 1: Demographics */}
          <div>
            <h3 className="text-sm font-bold uppercase tracking-wider text-[#1F5B5B] border-b border-[#D8CDBB]/40 pb-2 mb-4">
              Demographics
            </h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Full Name</label>
                <input
                  type="text"
                  required
                  value={fullName}
                  ...
                  className="w-full rounded-xl border border-[#D8CDBB] bg-[#FBF6EE]/40 px-4 py-2.5 text-sm text-[#24322F] focus:border-[#1F5B5B] focus:outline-none focus:ring-1 focus:ring-[#1F5B5B] transition-all"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Age</label>
                <input
                  type="number"
                  required
                  min={18}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Gender</label>
                <select
                  required
                  value={gender}
                  onChange={(e) => setGender(e.target.value)}
                  className="w-full rounded-xl border border-[#D8CDBB] bg-white px-4 py-2.5 text-sm text-[#24322F] focus:border-[#1F5B5B] focus:outline-none focus:ring-1 focus:ring-[#1F5B5B]"
                >
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Height (cm)</label>
                <input
                  type="number"
                  required
                  min={50}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Weight (kg)</label>
                <input
                  type="number"
                  required
                  min={10}
              ...
              <div className="flex flex-col justify-end">
                {bmiInfo ? (
                  <div className="rounded-xl bg-[#E5E9E7] border border-[#3A4B47]/10 p-3 flex justify-between items-center h-10.5">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#3A4B47]">BMI: {bmiInfo.value}</span>
                    <span className="text-[10px] font-bold text-[#1F5B5B] bg-[#DCEEE6] px-2 py-0.5 rounded-full">
                      {bmiInfo.category}
                    </span>
                  </div>
                ) : (
                  <div className="rounded-xl border border-dashed border-[#D8CDBB] h-10.5 flex items-center justify-center text-[10px] text-[#6E8B8B] font-semibold">
                    Awaiting vitals...
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-2">Pre-existing Conditions</label>
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all cursor-pointer ${
                          isSelected
                            ? "bg-[#1F5B5B] border-[#1F5B5B] text-white"
                            : "bg-white border-[#D8CDBB] text-[#6E8B8B] hover:border-[#1F5B5B]/40"
                        }`}
                      >
                        {opt}
                      </button>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all cursor-pointer ${
                      preExisting.includes("None")
                        ? "bg-[#3A4B47] border-[#3A4B47] text-white"
                        : "bg-white border-[#D8CDBB] text-[#6E8B8B] hover:border-[#1F5B5B]/40"
                    }`}
                  >
                    None
              ...
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-2">Family Medical History</label>
                <div className="flex flex-wrap gap-2">
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Smoking Status</label>
                <select
                  required
                  value={smokingStatus}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Activity Level</label>
                <select
                  required
                  value={activityLevel}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#24322F] mb-1.5">Dietary Preference</label>
                <select
                  required
