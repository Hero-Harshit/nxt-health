...
export async function POST(req: NextRequest) {
  ...
  if (!gmailUser || !gmailAppPassword) {
    console.error(
      "❌ [SOS Alert API Error]: GMAIL_USER or GMAIL_APP_PASSWORD is missing from environment variables."
    );
  ...
  try {
    ...
    if (!audioBase64 || typeof audioBase64 !== "string") {
      ...
      return NextResponse.json(
        { success: false, error: "Missing or invalid audioBase64 string" },
        { status: 400 }
      );
    }
    ...
    if (!healthPassport || typeof healthPassport !== "object") {
      ...
      return NextResponse.json(
        { success: false, error: "Missing or invalid healthPassport data" },
        { status: 400 }
      );
    }
    ...
    if (!toEmail || typeof toEmail !== "string" || !toEmail.trim()) {
      ...
      return NextResponse.json(
        { success: false, error: "Missing destination email (toEmail) inside healthPassport." },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: `SMTP Verification Failed: ${verifyError.message || verifyError.toString()}` },
        { status: 500 }
      );
    }
    ...
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
        ...
        <div style="background-color: #fff1f2; border: 1.5px solid #fecdd3; border-radius: 12px; padding: 20px; margin-bottom: 24px; box-shadow: inset 0 2px 4px 0 rgba(0,0,0,0.02);">
          <h4 style="margin: 0 0 8px 0; font-size: 11px; font-weight: 800; color: #be123c; text-transform: uppercase; letter-spacing: 0.8px;">🔴 EMERGENCY RECORDING</h4>
          <p style="margin: 0; font-size: 14px; color: #881337; line-height: 1.6; font-weight: 600;">
            An emergency audio recording (max 15 seconds) is attached to this email.
          </p>
        </div>
    ...
    console.log(`✉️ [Next.js SOS API]: Sending emergency email alert to ${toEmail}...`);
    ...
    console.log("✅ [Next.js SOS API]: Email successfully dispatched.");

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error("❌ Next.js SOS API Route Failed:", error);
    return NextResponse.json(
      {
        success: false,
